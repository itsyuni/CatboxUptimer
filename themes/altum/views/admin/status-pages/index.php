...
<div class="d-flex justify-content-between mb-4">
    <h1 class="h3"><i class="fa fa-fw fa-xs fa-wifi text-primary-900 mr-2"></i> <?= language()->admin_status_pages->header ?></h1>
        ...
        <div class="ml-3">
            <div class="dropdown">
                ...
                <div class="dropdown-menu  dropdown-menu-right">
                    <a href="<?= url('admin/status-pages?' . $data->filters->get_get() . '&export=csv') ?>" target="_blank" class="dropdown-item">
                        <i class="fa fa-fw fa-sm fa-file-csv mr-1"></i> <?= language()->global->export_csv ?>
                    </a>
                    <a href="<?= url('admin/status-pages?' . $data->filters->get_get() . '&export=json') ?>" target="_blank" class="dropdown-item">
                ...
                <div class="dropdown-menu dropdown-menu-right filters-dropdown">
                    <div class="dropdown-header d-flex justify-content-between">
                        ...
                        <?php if(count($data->filters->get)): ?>
                            <a href="<?= url('admin/status_pages') ?>" class="text-muted"><?= language()->global->filters->reset ?></a>
                        <?php endif ?>
                    </div>
                    ...
                    <form action="" method="get" role="form">
                        ...
                        <div class="form-group px-4">
                            ...
                            <select name="search_by" id="search_by" class="form-control form-control-sm">
                                <option value="name" <?= $data->filters->search_by == 'name' ? 'selected="selected"' : null ?>><?= language()->admin_status_pages->filters->search_by_name ?></option>
                            </select>
                        </div>
                        ...
                        <div class="form-group px-4">
                            ...
                            <select name="order_by" id="order_by" class="form-control form-control-sm">
                                ...
                                <option value="name" <?= $data->filters->order_by == 'name' ? 'selected="selected"' : null ?>><?= language()->admin_status_pages->filters->order_by_name ?></option>
                                <option value="pageviews" <?= $data->filters->order_by == 'pageviews' ? 'selected="selected"' : null ?>><?= language()->admin_status_pages->filters->order_by_pageviews ?></option>
                            </select>
                        </div>
...
<div class="table-responsive table-custom-container">
    <table class="table table-custom">
        <thead>
        <tr>
            <th><?= language()->admin_status_pages->table->user ?></th>
            <th><?= language()->admin_status_pages->table->name ?></th>
            <th><?= language()->admin_status_pages->table->stats ?></th>
            <th><?= language()->admin_status_pages->table->is_enabled ?></th>
            <th><?= language()->admin_status_pages->table->datetime ?></th>
            <th></th>
        </tr>
        </thead>
        <tbody>
        <?php foreach($data->status_pages as $row): ?>
            <tr>
                ...
                <td>
                    <?= $row->name ?><a href="<?= url('status-page-redirect/' . $row->status_page_id) ?>" rel="noreferrer" target="_blank"><i class="fa fa-fw fa-xs fa-external-link-alt ml-1"></i></a>
                </td>
                <td>
                    <div class="d-flex flex-column">
                        <span class="text-muted"><?= sprintf(language()->admin_status_pages->table->pageviews, nr($row->pageviews)) ?></span>
                    </div>
                </td>
                <td>
                ...
                <td>
                    <?= include_view(THEME_PATH . 'views/admin/status-pages/admin_status_page_dropdown_button.php', ['id' => $row->status_page_id]) ?>
                </td>
            </tr>
        <?php endforeach ?>
